...
def _render_figure1(pan_settings, pan_preview, active_res, glob, auto_bounds):
    with pan_settings:
        with st.expander("📈 Fig 1: Relaxation Curves", expanded=False):
            ...
            if show_fig1:
                ...
                if rel_custom_lims:
                    ...
                    with xlim_col1: rel_xmin = st.number_input("X Min (Time)", value=auto_bounds['auto_rel_xmin'], format="%.3e", key="pub_rel_xmin")
                    with xlim_col2: rel_xmax = st.number_input("X Max (Time)", value=auto_bounds['auto_rel_xmax'], format="%.3e", key="pub_rel_xmax")
                    with ylim_col1: rel_ymin = st.number_input("Y Min (Modulus)", value=auto_bounds['auto_rel_ymin'], format="%.3e", key="pub_rel_ymin")
                    with ylim_col2: rel_ymax = st.number_input("Y Max (Modulus)", value=auto_bounds['auto_rel_ymax'], format="%.3e", key="pub_rel_ymax")
    ...
    if show_fig1:
        with pan_preview:
            ...
            with plt.rc_context({'font.family': rel_num_family, 'xtick.labelsize': rel_tick_size, 'ytick.labelsize': rel_tick_size, 'axes.unicode_minus': False}):
                fig1, ax1 = plt.subplots(figsize=(glob['fig_width'] / 2.54, glob['fig_height'] / 2.54), facecolor='white')
                ...
                if panel_l_1:
                    ax1.text(pl_x_1, pl_y_1, f"({panel_l_1})", transform=ax1.transAxes,
                             fontfamily=glob['panel_font_family'], fontsize=glob['panel_font_size'],
...
def _render_figure2(pan_settings, pan_preview, active_res, kinetics_df, glob, auto_bounds, G_prime_input):
    if kinetics_df.empty: return
    with pan_settings:
        with st.expander("🔥 Fig 2: Tau Kinetics (Arrhenius / VFT)", expanded=False):
            ...
            if show_fig2:
                ...
                if kin_custom_lims:
                    ...
                    with kxlim_col1: kin_xmin = st.number_input("X Min (1000/T)", value=auto_bounds['auto_kin_xmin'], format="%.4f", key="pub_kin_xmin")
                    with kxlim_col2: kin_xmax = st.number_input("X Max (1000/T)", value=auto_bounds['auto_kin_xmax'], format="%.4f", key="pub_kin_xmax")
                    with kylim_col1: kin_ymin = st.number_input("Y Min (ln(τ))", value=auto_bounds['auto_kin_ymin'], format="%.4f", key="pub_kin_ymin")
                    with kylim_col2: kin_ymax = st.number_input("Y Max (ln(τ))", value=auto_bounds['auto_kin_ymax'], format="%.4f", key="pub_kin_ymax")
        ...
        with pan_preview:
            ...
            if not active_k.empty and len(active_k) >= 2:
                ...
                if fit_res_pub:
                    ...
                    with plt.rc_context({'font.family': kin_num_family, 'xtick.labelsize': kin_tick_size, 'ytick.labelsize': kin_tick_size, 'axes.unicode_minus': False}):
                        fig2, ax2 = plt.subplots(figsize=(glob['fig_width'] / 2.54, glob['fig_height'] / 2.54), facecolor='white')
                        ax2.grid(False)
                        for spine in ['top', 'bottom', 'left', 'right']:
...
def _render_figure3(pan_settings, pan_preview, kinetics_df, glob, auto_bounds):
    if kinetics_df.empty: return
    with pan_settings:
            ...
            if not active_k.empty and len(active_k) >= 2:
                ...
                if fit_res_pub:
                    ...
                    with plt.rc_context({'font.family': eyr_tick_font if eyr_tick_font != 'Same as Label' else eyr_font_family, 'xtick.labelsize': eyr_tick_size, 'ytick.labelsize': eyr_tick_size, 'axes.unicode_minus': False}):
                        fig3, ax3 = plt.subplots(figsize=(glob['fig_width'] / 2.54, glob['fig_height'] / 2.54), facecolor='white')
                        ax3.grid(False)
                        for spine in ['top', 'bottom', 'left', 'right']:
...
def _render_figure4(pan_settings, pan_preview, active_res, kinetics_df, glob, auto_bounds):
    if kinetics_df.empty: return
    with pan_settings:
        with st.expander("🌡️ Fig 4: Van 't Hoff (Decrosslinking)", expanded=False):
            ...
            if show_fig4:
                ...
                if vh_custom_lims:
                    ...
                    with vx1: vh_xmin = st.number_input("X Min", value=auto_bounds['auto_kin_xmin'], format="%.4f", key="vh_xmin")
                    with vx2: vh_xmax = st.number_input("X Max", value=auto_bounds['auto_kin_xmax'], format="%.4f", key="vh_xmax")
                    with vy1: vh_ymin = st.number_input("Y Min", value=auto_bounds['auto_vh_ymin'], format="%.4f", key="vh_ymin")
                    with vy2: vh_ymax = st.number_input("Y Max", value=auto_bounds['auto_vh_ymax'], format="%.4f", key="vh_ymax")
            ...
            if not active_k.empty and len(active_k) >= 2:
                ...
                if fit_res_pub:
                    ...
                    with plt.rc_context({'font.family': vh_tick_font if vh_tick_font != 'Same as Label' else vh_font_family, 'xtick.labelsize': vh_tick_size, 'ytick.labelsize': vh_tick_size, 'axes.unicode_minus': False}):
                        fig4, ax4 = plt.subplots(figsize=(glob['fig_width'] / 2.54, glob['fig_height'] / 2.54), facecolor='white')
                        ax4.grid(False)
                        for spine in ['top', 'bottom', 'left', 'right']:
...
def render_publication(tab_pub, PLOTLY_STYLE: dict, Tg_input: float, G_prime_input: float):
    with tab_pub:
        ...
        if 'active_results' in st.session_state and st.session_state.active_results:
            ...
            auto_bounds = {
                'auto_rel_xmin': auto_rel_xmin, 'auto_rel_xmax': auto_rel_xmax,
                'auto_rel_ymin': auto_rel_ymin, 'auto_rel_ymax': auto_rel_ymax,
                'auto_kin_xmin': auto_kin_xmin, 'auto_kin_xmax': auto_kin_xmax,
                'auto_kin_ymin': auto_kin_ymin, 'auto_kin_ymax': auto_kin_ymax,
                'auto_vh_ymin': auto_vh_ymin, 'auto_vh_ymax': auto_vh_ymax
            }
            ...
            _render_figure2(pan_settings, pan_preview, active_res, kinetics_df, glob, auto_bounds, G_prime_input)
            _render_figure3(pan_settings, pan_preview, kinetics_df, glob, auto_bounds)
            _render_figure4(pan_settings, pan_preview, active_res, kinetics_df, glob, auto_bounds)
